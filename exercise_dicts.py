...
def list_inventory(inventario):
    ...
    respuesta = []
    tuplas = list(inventario.items())
    for item, cantidad in tuplas:
        if cantidad> 0:
            respuesta.append((item, cantidad))
    return respuesta
...
def find_max_value(diccionario):
    ...
    nombre, puntajee = tupla[0]

    for persona, puntaje in tupla:
        if puntaje > puntajee:
            puntaje = puntajee
            nombre = persona
    return nombre
...
def find_biggest_expense(gastos):
    ...
    for categoria, lista_gastos in gastos.items():
        total_categoria = sum(lista_gastos)
...
def sum_expenses(gastos):
    ...
    totales = {}
    for categoria, lista_gastos in gastos.items():
        totales[categoria] = sum(lista_gastos)
    return totales
...
def sum_expenses_by_type(gastos):
    ...
    """
    diccionario = dict()
    for categorias in gastos:
        tuplas =gastos[categorias]
        for tipo, monto in tuplas:
